...
while robot.step(timestep) != -1:
    ...
    if m_line == None:
        m_line = q1.get_line_through_points(current_location, goal_location)
    ...
    if mode == 0:
        ...
        if (float(q1.make_Point(hit_point).distance(q1.make_Point(current_location))) >= 0.05):
            temp_angle = float(m_line.angle_between(Ray(hit_point, current_location)))
        else:
            temp_angle=0
        if (temp_angle < 0.1 or (temp_angle>3.04 and temp_angle< 3.24)) and (float(q1.make_Point(hit_point).distance(q1.make_Point(current_location))) >= 0.05):
            if m_line.p1.distance(current_location)< m_line.p1.distance(goal_location):
                mode = 0
